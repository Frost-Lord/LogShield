...
const wafRulesPath = path.join(__dirname, '../waf-rules.json');
...
function checkAgainstWAFRules(value) {
  for (const rule of wafRules) {
    ...
    if (regex.test(value)) {
      logger.info('WAF', `Rule ${rule.id}: "${rule.cmt}" in category "${rule.type}" has been triggered by request at ${new Date().toISOString()}`);
      return true;
    }
  }
  return false;
